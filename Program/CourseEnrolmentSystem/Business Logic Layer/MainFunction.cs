...
namespace CourseEnrolmentSystem.Business_Logic_Layer
{
    internal class MainFunction
    {
                              ...
                              TableLayoutPanel coursesAvailableTable, Label availability)
        {
            ...
            {               
                for (int i = 1; i <= 5; i++)
                {
                    ...
                    if (!_subjects.Any(s => s.Name == subjectName))
                    {
                        _subjects.Add(new Subject(subjectName, grade));
                        if (_subjects.Count == 5) isValid = true;
                    }
                    else
                    {
                        MessageBox.Show($"Subject '{subjectName}' already exists. Please choose a different subject.\nCleared duplicate field");
                        subjectName = "";
                        switch (i)
                        {
                            case 1:
            ...
            if (isValid)
            {
                List<string> courseAvailable = DatabaseBll.GetAvailableCourses(_subjects, TotalPoints());
        ...
        {
            return Points.CalculatePoints(_subjects);
        }
